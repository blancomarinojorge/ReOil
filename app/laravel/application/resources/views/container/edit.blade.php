<x-layout>
    <x-global.top-bar :title="__('Edit client container')" :breadcrumbs-items="[
        ...
        ['label'=>$container->containerType->name, 'href'=>route('containers.show', $container->id)],
        ['label'=>__('Edit')]
    ]">
    </x-global.top-bar>
    <article class="p-4">
        <x-forms.form method="PUT" action="{{ route('containers.update', $container->id) }}" class="flex flex-col max-w-300 gap-8 lg:mx-auto my-15 mx-5">
                        ...
                        <x-forms.field :label="__('client')" name="client_id" :required="true">
                            <select class="py-3 pl-5 border-1 border-tertiary/40 focus:border-tertiary/70" name="client_id" id="client_id">
                                ...
                                @foreach($clients as $client)
                                    <option value="{{ $client->id }}" @if(old('client_id', $container->client_id) == $client->id) selected @endif>{{ $client->name }}</option>
                                @endforeach
                            </select>
                        </x-forms.field>
                        <x-forms.field :label="__('container type')" name="container_type_id" :required="true">
                            <select class="py-3 pl-5 border-1 border-tertiary/40 focus:border-tertiary/70" name="container_type_id" id="container_type_id">
                                ...
                                @foreach($containerTypes as $containerType)
                                    <option value="{{ $containerType->id }}" @if(old('container_type', $container->containerType->id) == $containerType->id) selected @endif>{{ $containerType->name }}</option>
                                @endforeach
                            </select>
                        </x-forms.field>
                    </div>
                    <div class="grid">
                        <x-forms.textarea :label="__('observations')" name="observations" placeholder="This container suffered damage in wend..." :value="$container->observations" class="h-60"/>
                    </div>
                </div>
            </div>
