<x-layout>
    <x-global.top-bar :title="__('New residue')" :breadcrumbs-items="[
        ['label'=>__('Residues'), 'href'=>route('residues.index')],
        ['label'=>__('New residue')]
    ]">
    </x-global.top-bar>
    <article class="p-4">
        <x-forms.form method="POST" action="{{ route('residues.store') }}" class="flex flex-col max-w-300 gap-8 lg:mx-auto my-15 mx-5">
            <div class="flex flex-col gap-3">
                ...
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <x-forms.input :label="__('name')" name="name" :required="true" class="lg:col-span-2"/>
            ...
            <div class="flex gap-3 justify-end">
                <x-link-button href="{{ route('residues.index') }}" type="secondary">{{ __('Cancel') }}</x-link-button>
